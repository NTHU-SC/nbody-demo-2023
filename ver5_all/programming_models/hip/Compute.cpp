...
void GSimulation :: start() 
{
  ...
  hipMemcpy(particles_mass_d, particles->mass, n * sizeof(real_type), hipMemcpyHostToDevice);

  hipMemcpy(particles_pos_x_d, particles->pos_x, n * sizeof(real_type), hipMemcpyHostToDevice);
  hipMemcpy(particles_pos_y_d, particles->pos_y, n * sizeof(real_type), hipMemcpyHostToDevice);
  hipMemcpy(particles_pos_z_d, particles->pos_z, n * sizeof(real_type), hipMemcpyHostToDevice);

  hipMemcpy(particles_acc_x_d, particles->acc_x, n * sizeof(real_type), hipMemcpyHostToDevice);
  hipMemcpy(particles_acc_y_d, particles->acc_y, n * sizeof(real_type), hipMemcpyHostToDevice);
  hipMemcpy(particles_acc_z_d, particles->acc_z, n * sizeof(real_type), hipMemcpyHostToDevice);
  ...
  {   
    ...
    hipLaunchKernelGGL(nbody, dim3(grid_size), dim3(block_size), 0, 0, particles_pos_x_d, particles_pos_y_d,
                                    particles_pos_z_d,
                                    particles_acc_x_d, particles_acc_y_d,
                                    particles_acc_z_d, particles_mass_d, n);

    hipMemcpy(particles->acc_x, particles_acc_x_d, n * sizeof(real_type), hipMemcpyDeviceToHost);
    hipMemcpy(particles->acc_y, particles_acc_y_d, n * sizeof(real_type), hipMemcpyDeviceToHost);
    hipMemcpy(particles->acc_z, particles_acc_z_d, n * sizeof(real_type), hipMemcpyDeviceToHost);
